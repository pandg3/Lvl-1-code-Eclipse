...
Boolean random = new Random().nextBoolean();
	...
	private void makeAlbum() {
		...
		JOptionPane.showMessageDialog(null,
				"To play the game, press the key that is pictured");
...
JFrame Siri2 = new JFrame();
		// 6. set the frame to visible
Siri2.setVisible(true);
		 Siri2.add(getNextRandomImage()); //7. rename to the name of your
		// frame

		// 8. set the size of the frame
Siri2.setSize(200,200);
		// 9. add a key listener to the frame
Siri2.addKeyListener(this);
...
System.out.println(random);
	...
	private Component getNextRandomImage() {
		...
		;
		if (random){
			speak("Simon says "+(images.get(imageIndex)));
			
		}
		else{
			speak ((images.get(imageIndex)));
		}
		if (imageIndex == 37){
